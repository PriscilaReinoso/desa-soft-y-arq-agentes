...
inventario no eliminados, con paginación. Cada ítem incluye `id`,
`articulo_id`, `medida_id`, `espacio_id`, `fila`, `columna`, `stock`,
`precio_venta` y las marcas de auditoría `created_at`, `updated_at`,
`deleted_at`.
...
`id`. Si el ítem no existe o fue eliminado, responde HTTP 404.

#### Scenario: Ítem existente
- **WHEN** se consulta un ítem por un `id` válido y no eliminado
- **THEN** el sistema responde HTTP 200 con los datos del ítem
