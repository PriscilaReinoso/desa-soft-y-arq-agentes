...
reciba `email` y `password`, valide las credenciales contra el usuario activo
y no eliminado y devuelva HTTP 200 con un token JWT de acceso, el tipo de
token, los segundos de expiración y los datos del usuario (id, nombre,
apellido, email y rol). Si las credenciales son inválidas, el usuario está
inactivo o fue eliminado, responde HTTP 401 sin revelar cuál condición se
incumplió.

#### Scenario: Login exitoso
- **WHEN** se envían un `email` y `password` correctos de un usuario activo y no eliminado
- **THEN** el sistema responde HTTP 200 con `access_token`, `token_type`, `expires_in` y los datos del usuario

#### Scenario: Credenciales inválidas
- **WHEN** se envían un `email` o `password` incorrectos
...
- **WHEN** la solicitud no incluye `email` o `password`
