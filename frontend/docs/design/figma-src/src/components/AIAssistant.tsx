        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>
              Consultas inteligentes sobre tu inventario
            </div>
          </div>
          <span
            style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
          {suggestions.map((s) => (
            <button
              ...
              onMouseLeave={(e) => ((e.target as HTMLElement).style.background = 'var(--muted)')}
            >
              {s}
            </button>
          ))}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 36px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {loading && (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg, #4A6B8A 0%, #3A5A7A 100%)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}>✦</div>
